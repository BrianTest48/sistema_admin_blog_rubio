<?php
    ...
    switch($_GET["op"]){
        ...
        case "listar_tabla":
            $datos=$banner->get_banner();
            $data = Array();
            foreach ($datos as $row) {
                $sub_array = array();
                $sub_array[] = $row["id"];
                $sub_array[] = '<img src="../../assets/portada/'.$row["imagen"].'" width="120px" height="100px">';
        ...
        case 'guardaryeditar':
            ...
            $datos = $banner->get_banner_id($_POST["ptd_id"]);
            if (empty($_POST["ptd_id"])) {
                if (is_array($datos) && count($datos) == 0) {
                    $banner->insert_banner($nombre_imagen);
                    move_uploaded_file($_FILES['ptd_image']['tmp_name'], $ruta_archivo);
                }
            } else {
                $banner->update_banner($_POST["ptd_id"], $nombre_imagen);
                move_uploaded_file($_FILES['ptd_image']['tmp_name'], $ruta_archivo);
            }
        ...
        case "mostrar":
            ...
            break;
        case "eliminar":
            $resultado = $banner->delete_banner($_POST["id"]);
